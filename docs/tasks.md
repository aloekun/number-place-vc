...
- [ ] 10.19 数字の影響範囲可視化機能（ハイライト機能）の実装
  - 機能概要: 選択中の数字と同じ行・列・ブロックの空きマスをハイライト表示
  - UIの追加:
    - mode-controlsセクションにトグルスイッチを追加（ラベル: 「範囲表示」）
    - 通常入力/候補入力ボタンの右側に配置
  - スタイルの実装:
    - トグルスイッチのデザイン（チェックボックスベースのスライド式）
    - ハイライト用クラス.highlight-rangeの定義（背景色: var(--primary-dark)）
  - 状態管理（GameState.js）:
    - highlightRangeEnabledプロパティの追加
    - toggleHighlightRange()メソッドの実装
  - ハイライト対象計算（Grid.js）:
    - getHighlightCells(number)メソッドの追加
    - 同じ行・列・ブロックの空きマス座標を返す
  ...
  - 動作仕様:
    - トグルON時のみハイライト表示
    - 空きマス（given/user-input以外）のみ対象
    - 数字未選択時はハイライトなし
  - _Requirements: ゲームプレイ支援、戦略的思考の補助、視覚的フィードバック向上_
